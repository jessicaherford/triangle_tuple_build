...
using System.Linq;
namespace triangle_tuple_build
{
    public class AllTriangles
    {
        ...
        private static object M(Tuple<string, int[], int[], int[]> tuple)
        {
            ...
            foreach (KeyValuePair<string, List<int>> pair in TriangleDict)
            {
                ...
                //Need to seperate this outside of foreach loop
                //if(TriangleDict.ContainsKey(userKey) == false || tuple.Item2[0].Equals(userCoords[0]) == false && tuple.Item2[1].Equals(userCoords[1]) == false && tuple.Item3[0].Equals(userCoords[2])== false && tuple.Item3[1].Equals(userCoords[3])== false && tuple.Item4[0].Equals(userCoords[4])== false && tuple.Item4[1].Equals(userCoords[5])== false)
                //{
                //    Console.WriteLine("Triangle Key or Coordinates do not exist.");
                //}
            }
            return TriangleDict;
